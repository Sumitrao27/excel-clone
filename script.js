...
if(checkLoop(rowId,colId)){
    const options={
        type:'question',
        buttons:['ok','cancel'],
        defaultId:0,
        title:'question',
        message:'There are one or more circular references where a formula refers to its own cell either directly or indirectly.',
        detail:'Try removing or changing these references ,or moving the formulas to different cell',
    }
    dialog.showMessageBox(null,options,(respose)=>{
...
function checkLoop(r,c){
    ...
    if(loop(r,c,visit,rec)==true){
        return true;
    }
    return false;
}
function loop(r,c,visit,rec){
     if(visit[r][c].val==false){
         visit[r][c].val=true;
         rec[r][c].val=true;
         let cell=getCellObject(r,c);
         for(let i=0;i<cell.downstream.length;i++){
            let curr=cell.downstream[i];
            if(!visit[curr.rowId][curr.colId].val && loop(curr.rowId,curr.colId,visit,rec)){
                return true;
            }
            else if(rec[curr.rowId][curr.colId].val){
                return true;
            }
         }
     }
